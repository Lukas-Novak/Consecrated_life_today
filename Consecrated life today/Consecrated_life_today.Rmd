...
#.......................
```
  In the next step, we compared clerics with representative sample in the General health. It was revealed that (`r inline_text(ra.e.2, variable = Source, pattern = "OR {estimate}; 95% CI ({conf.low}, {conf.high}); {p.value}")`).. <!-- As this is the first use of the varible GH and chronotype, there is no need to use Bonferroni --> However, in adjusted effect, this result was non significant (`r inline_text(ra.e.1, variable = Source, pattern = "OR {estimate}; 95% CI ({conf.low}, {conf.high}); {p.value}")`).  

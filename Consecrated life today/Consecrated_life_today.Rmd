...
# data.rep= data.rep %>% 
#   select(marital_status_all_groups,sex,economical_status_all_groups,education,
#          Faith, CISLO_DOTAZNIKU,IHD, "IBD", "asthma",
# "diabetes", "Vek",
#  "artritis",
# "back_pain", 
# "allergy",
...
#  "cancer","DSES1","DSES15") 
# 
# data.rep %>% saveRDS(file = "data.rep.Rds")

data.rep = readRDS("data.rep.Rds")
#(....................................................................................)
...
#   select(starts_with(c("code","low_q_res","economical_status","education","gender","age","family_status","obtize_","hlth_","daily_energy")))
# 
# data.pa.raw %>% saveRDS(file = "data.pa.raw.Rds")

data.pa.raw = readRDS("data.pa.raw.Rds")
...
"daily_energy", "Family_status","code","Source")))

# summ of all diseases
...
MCAR.pa.data.mis.mech=MissMech::TestMCARNormality(data = select(.data = data.pa, starts_with(c("Age","hlth_"))))
...
# mvn(data = select(.data = data.pa, starts_with(c("Age","daily_energy","psychotherapy","hlth_"))), mvnTest = "mardia", univariatePlot = "histogram")
    
# inspection indicated skewness problems with items: 
Mard=mvn(data= select(.data = data.pa, starts_with(c("Age","hlth_"))),mvnTest = "mardia")    
...
fake_regres=lm(random~., data = select(data.pa, starts_with(c("Age", "obtize_","daily_energy","psychotherapy","hlth_", "Gender", "Education")))) # run fake regression
...
# data.cons.long = data.cons.long %>% 
#   select(starts_with(c("duplicities","hlth_","obtize_","religious_age","low_q_res","daily_energy","country","Gender","Gender","Education","Age","Economical_status","code")))
# 
# 
# data.cons.long %>% saveRDS(file = "data.cons.long.Rds")

data.cons.long = readRDS("data.cons.long.Rds")
...
data.cons.raw = data.cons.raw %>% 
  ...
  mutate(across(starts_with("obtize_"), ~ dplyr::recode(.,
                                                ...
                                                "4" = "Many times a week",
                                                "5" = "Many times a week"))) %>% 
  mutate(across(starts_with(c("hlth_","religious_age")), ~ as.numeric(.))) %>% 
    mutate(Education = recode_factor(Education, 
                                   "Basic school" = "Basic school", 
                                   "Vocational school or non - maturity high school" = "Vocational school or non - maturity high school",
                                   "High school" = "High school",
...
"obtize_1a", "obtize_2a", "obtize_3a", "obtize_4a", "obtize_5a", 
"obtize_6a", "obtize_7a", "obtize_8a", "obtize_9a", "obtize_10a", "obtize_11a", "obtize_12a",
"daily_energy","country","code","religious_age","code","Source"))) %>% 
...
MCAR.cons.data.mis.mech=MissMech::TestMCARNormality(data = select(.data = data.cons, starts_with(c(
                                                                                                 "hlth_","Age",
                                                                                                 "daily_energy",
                                                                                                 "psychotherapy",
                                                                                                 "religious_age"))))
...
# MULTIVARIATE NORMALITY:
# mvn(data = select(.data = data.cons, starts_with(c("obtize_","hlth_","Age","religious_age"))), mvnTest = "mardia", univariatePlot = "histogram")
# inspection indicated skewness problems with items: 
Mard=mvn(data= select(.data = data.cons, starts_with(c(
...
fake_regres=lm(random~., data= data.cons %>% select(starts_with(c("obtize_",

...
## Chronic illness differences
  Regression analysis indicated that there is a significant positive relationship between being a consecrated person and lower probability of suffering from chronic illnesses such as hypertension, diabetes or arthritis. However, there was positive relationship between being cleric and depression/anxiety see table 3 for more details.  
...
vars.prev.tab  %>% 
  ...
  statistic = list(all_continuous() ~  "{mean} ({sd})",
                                   all_categorical() ~ "{n} ({p}%)"),
              digits = all_continuous() ~ 2) %>% 
  modify_footnote(update = everything() ~ "In the table") %>% 
...
# Side-by-side Regression Models
# logistic regression model
# we have 46 analysis in total thus 0.05/52 = 0.0009615385 = criterion P-value
...
#_____________________________________________________________
ra.b.9 <-
  glm(obtize_5a ~ Source + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.2) %>%
  tbl_regression(exponentiate = TRUE, include = "Source", show_single_row = "Source", 
                 label = list(Source ~ "Feeling depressed"))%>%
  bold_p(t = 0.0009615385) 
ra.b.10 <-
  glm(obtize_5a ~ Source, family = binomial, data = data.reg.2) %>%
  tbl_regression(exponentiate = TRUE, include = "Source", show_single_row = "Source", 
                 label = list(Source ~ "Feeling depressed"))%>%
  bold_p(t = 0.0009615385) 
#_____________________________________________________________
ra.b.11 <-
  glm(obtize_6a ~ Source + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.2) %>%
  tbl_regression(exponentiate = TRUE, include = "Source", show_single_row = "Source", 
                 label = list(Source ~ "Irritability, bad mood"))%>%
  bold_p(t = 0.0009615385) 
ra.b.12 <-
  glm(obtize_6a ~ Source, family = binomial, data = data.reg.2) %>%
  tbl_regression(exponentiate = TRUE, include = "Source", show_single_row = "Source", 
                 label = list(Source ~ "Irritability, bad mood"))%>%
  bold_p(t = 0.0009615385)
#_____________________________________________________________
ra.b.13 <-
  glm(obtize_7a ~ Source + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.2) %>%
  tbl_regression(exponentiate = TRUE, include = "Source", show_single_row = "Source", 
                 label = list(Source ~ "Nervousness"))%>%
  bold_p(t = 0.0009615385) 
ra.b.14 <-
  glm(obtize_7a ~ Source, family = binomial, data = data.reg.2) %>%
  tbl_regression(exponentiate = TRUE, include = "Source", show_single_row = "Source", 
                 label = list(Source ~ "Nervousness"))%>%
  bold_p(t = 0.0009615385) 
#_____________________________________________________________
...
tr.b.1=tab.wide.3[,c(0:5)] %>% as_tibble(rownames = NA)
tr.b.4=tab.wide.3[,c(6:10)] %>% as_tibble(rownames = NA)         

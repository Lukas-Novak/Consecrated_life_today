...
### Long lasting ilnessess
  For the occurrence of chronic illnesses, a 18-item measure was used introduced by the question: “Do you have a long-term illness or disability? Please tick all that apply to you.“ The illnesses are listed in table 3. 
  
### General health (GH)
  The GH was assessed by composite variable created by summing up number of chronic illnesses. This variable was consequently divided into several categories based on the following key:   
...
## Statistical analysis
  ...
  To explore differences in health status among clerics and non - clerics, we compared in logistic regression models long lasting illnesses of clerics to reported long lasting illnesses of participants from representative sample. In these models, reported long lasting illness was was as a dependent variable, grouping variable distinguishing clerics from non - clerics was regressor and covariates were are, gender, education and length of a life in clerical order. Multinominal regression was used to compare clerincs and non - clerics to composite variable created as summ of all illnesses and reflecting general health.       
...
The table 1 depicts basic socio-demographic characteristics of the study samples. 
...
```{r contingenty tables, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
# library(sjPlot)
...
The table 3 presents differences in chronic diseases between clerics and representative sample. It was revealed that there is a significant positive relationship between being a consecrated person and lower probability of chronic illnesses such as hypertension, diabetes or arthritis in adjusted effect. However, there was positive relationship between being cleric and obesity in crude effect and pain in the small pelvis in both crude and adjusted effect. After Bonferroni correction, no further significant results were found. In the table 4, there can be found prevalence of chronic diseases among study samples.
...
tr.1=tab.wide.1[,c(16,1:3,18)] %>% as_tibble(rownames = NA)
tr.4=tab.wide.1[,c(5,14:15,17)] %>% as_tibble(rownames = NA)         
tr.2=tab.wide.1[,c(6:9,19)] %>% as_tibble(rownames = NA)
tr.3=tab.wide.1[,c(10:12,13)] %>% as_tibble(rownames = NA) 
         
         tab = bind_rows(tr.1,tr.4,tr.2,tr.3)
         tab = tab %>% 
...
vars.prev.tab  %>% 
  ...
  statistic = list(all_continuous() ~  "{mean} ({sd})",
                                   all_categorical() ~ "{n} ({p}%)"),
              digits = all_continuous() ~ 2) %>% 
  modify_footnote(update = everything() ~ "ICHS = Ischemic heart disease, 4a = Cleric sample consisting of Czech participants, 4b = Cleric sample consisting of Slovak participants") %>% 
  modify_caption("Prevalence of chronical ilnesses among study samples") %>% 
  as_kable_extra(format = "latex",
                 booktabs = TRUE) %>% 
...
ra.b.1 <-
  glm(obtize_1a ~ Source + Age + Education  + Gender + religious_age.y, family = binomial, data = data.reg.2) %>%
  tbl_regression(exponentiate = TRUE, include = "Source", show_single_row = "Source", 
...
#_____________________________________________________________
ra.b.3 <-
  glm(obtize_2a ~ Source + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.2) %>%
  tbl_regression(exponentiate = TRUE, include = "Source", show_single_row = "Source", 
...
#_____________________________________________________________
ra.b.5 <-
  glm(obtize_3a ~ Source + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.2) %>%
  tbl_regression(exponentiate = TRUE, include = "Source", show_single_row = "Source", 
...
#_____________________________________________________________
ra.b.7 <-
  glm(obtize_4a ~ Source + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.2) %>%
  tbl_regression(exponentiate = TRUE, include = "Source", show_single_row = "Source", 
...
#_____________________________________________________________
ra.b.15 <-
  glm(obtize_8a ~ Source + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.2) %>%
  tbl_regression(exponentiate = TRUE, include = "Source", show_single_row = "Source", 
...
ra.b.17 <-
  glm(obtize_9a ~ Source + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.2) %>%
  tbl_regression(exponentiate = TRUE, include = "Source", show_single_row = "Source", 
...
<!-- end HC -->
## Health complains 
  Table 5 refers to associations between health complains in clerics as compared to representative sample. After Bonferroni correction, no result remained significant. However, there were some trends relatively close to significance threshold: for instance, clerics had higher odds of troubles falling into sleep in crude effect. 

```{r Print regres tab 2, echo=FALSE, message=FALSE, warning=FALSE}
  knitr::kable(tab2.b, "latex", booktabs=T, linesep = "", caption = "Depicts associations (in Odds rations) between living in clerical life and health complains in the last month") %>% 
  kableExtra::kable_styling(latex_options = c("scale_down","hold_position"),   font_size = 7) %>%  # full_width = T or longtable = T (to the Kable command)
...
## Chronotype and chronic illnesses
...
ra.c.1 <-
  glm(hlth_hypertension ~ Chronotype + Age + Education  + Gender + religious_age.y, family = binomial, data = data.reg.relig) %>%
...
#_____________________________________________________________
ra.c.3 <-
  glm(hlth_diabetes ~ Chronotype + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.relig) %>%
  tbl_regression(exponentiate = TRUE, include = "Chronotype", show_single_row = "Chronotype", 
...
#_____________________________________________________________
ra.c.5 <-
  glm(hlth_arthritis ~ Chronotype + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.relig) %>%
  tbl_regression(exponentiate = TRUE, include = "Chronotype", show_single_row = "Chronotype", 
...
#_____________________________________________________________
ra.c.7 <-
  glm(hlth_astma ~ Chronotype + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.relig) %>%
  tbl_regression(exponentiate = TRUE, include = "Chronotype", show_single_row = "Chronotype", 
...
#_____________________________________________________________
ra.c.9 <-
  glm(hlth_depression_anxiety ~ Chronotype + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.relig) %>%
  tbl_regression(exponentiate = TRUE, include = "Chronotype", show_single_row = "Chronotype", 
...
#_____________________________________________________________
ra.c.11 <-
  glm(hlth_ICHS ~ Chronotype + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.relig) %>%
  tbl_regression(exponentiate = TRUE, include = "Chronotype", show_single_row = "Chronotype", 
...
#_____________________________________________________________
ra.c.13 <-
  glm(hlth_obesity ~ Chronotype + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.relig) %>%
  tbl_regression(exponentiate = TRUE, include = "Chronotype", show_single_row = "Chronotype", 
...
#_____________________________________________________________
ra.c.17 <-
  glm(hlth_back_pain ~ Chronotype + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.relig) %>%
  tbl_regression(exponentiate = TRUE, include = "Chronotype", show_single_row = "Chronotype", 
...
#___________________________________________________________________________________________________________________
ra.c.19 <-
  glm(hlth_gastric_or_duodenal_ulcers ~ Chronotype + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.relig) %>%
  tbl_regression(exponentiate = TRUE, include = "Chronotype", show_single_row = "Chronotype", 
...
#_____________________________________________________________
ra.c.23 <-
  glm(hlth_skin_diseases_eczema ~ Chronotype + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.relig) %>%
  tbl_regression(exponentiate = TRUE, include = "Chronotype", show_single_row = "Chronotype", 
...
#_____________________________________________________________
ra.c.25 <-
  glm(hlth_allergy ~ Chronotype + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.relig) %>%
  tbl_regression(exponentiate = TRUE, include = "Chronotype", show_single_row = "Chronotype", 
...
#_____________________________________________________________
ra.c.27 <-
  glm(hlth_migraine ~ Chronotype + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.relig) %>%
  tbl_regression(exponentiate = TRUE, include = "Chronotype", show_single_row = "Chronotype", 
...
#_____________________________________________________________
ra.c.29 <-
  glm(hlth_pain_of_unclear_origin ~ Chronotype + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.relig) %>%
  tbl_regression(exponentiate = TRUE, include = "Chronotype", show_single_row = "Chronotype", 
...
#_____________________________________________________________
ra.c.31 <-
  glm(hlth_for_wom_pain_in_the_small_ ~ Chronotype + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.relig) %>%
  tbl_regression(exponentiate = TRUE, include = "Chronotype", show_single_row = "Chronotype", 
...
#_____________________________________________________________
ra.c.33 <-
  glm(hlth_cancer ~ Chronotype + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.relig) %>%
  tbl_regression(exponentiate = TRUE, include = "Chronotype", show_single_row = "Chronotype", 
...
#_____________________________________________________________
ra.c.35 <-
  glm(hlth_thyroid_disease ~ Chronotype + Age + Education + Gender + religious_age.y, family = binomial, data = data.reg.relig) %>%
  tbl_regression(exponentiate = TRUE, include = "Chronotype", show_single_row = "Chronotype", 
...
```{r convert gtsum tab into tible 3, include=FALSE}
table.3=ra.c.t1
...
```{r preparation to print tab 3, message=FALSE, warning=FALSE, include=FALSE}
...
```{r Print regres tab 3, echo=FALSE, message=FALSE, warning=FALSE}
...
## Chronotype and health complains
  In the table 7 there can be found associations between early birds and night owls in health complains in clerical sample. The following trends were found after Bonferroni correction: night owls had higher probability of problems of falling into sleep as compared to early birds (crude and adjusted effect).   

<!-- Health complains -->
```{r health complains table 4, include=FALSE}
# Side-by-side Regression Models
# logistic regression model
...
```{r convert gtsum tab into tible 4, include=FALSE}
table.4=ra.d.t2
...
```{r preparation to print tab 4, message=FALSE, warning=FALSE, include=FALSE}
# the only step here which is required here is to manually divide dataset into four parts and then rbind these togather 
...
```{r Print regres tab 4, echo=FALSE, message=FALSE, warning=FALSE}

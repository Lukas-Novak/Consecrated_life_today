...
gender, education and length of a life in clerical order. Proportional
odds model was used to compare clerincs and non - clerics to composite
variable created as summ of all illnesses and reflecting general health.
The same method was applied to explore associations between chronotype
and chronic illnesses and helth complains in clerics. Proportional odds
assumption in each of the ordinal regression model by the Brant test and
it was suggested that this assumption hodls for each model.
Multicolineariy was assessed by variance inflation factor (VIF) and its
results (VIF \textless{} 10 \citep{TAY2017}) did not suggest
multicolinearity.
...
\hypertarget{general-health-chronotype-and-comparison-between}{%
\subsection{General health, chronotype and comparison
between}\label{general-health-chronotype-and-comparison-between}}
...
CI (1.02, 2.30); p=0.039). . However, in adjusted effect, this result
was non significnat (OR 1.45; 95\% CI (0.96, 2.21); p=0.078).
...
effect).\\
OR 1.45; 95\% CI (0.96, 2.21); p=0.078

OR 1.53; 95\% CI (1.02, 2.30); p=0.039
...
\tightlist
\item
  další krok spočítat dvě série ordinální regrese pro chronotyp a
  chronická onemocnění - předešlé se tedy asi nepoužije
\end{itemize}

\hypertarget{sourceknitrpurlregres_chronotype_chron_ilness_both_binary.rmd-quiettrue}{%
\section{source(knitr::purl(``regres\_chronotype\_chron\_ilness\_both\_binary.rmd'',
quiet=TRUE))}\label{sourceknitrpurlregres_chronotype_chron_ilness_both_binary.rmd-quiettrue}}
